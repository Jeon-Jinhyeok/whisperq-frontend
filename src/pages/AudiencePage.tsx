...
export function AudiencePage() {
  ...
  useEffect(() => {
    const hasSeenOnboarding = localStorage.getItem(`whisperq_onboarding_${sessionCode}`);
  ...
  const handleReaction = useCallback(async (type: ReactionType, e: React.MouseEvent) => {
    const now = Date.now();
        ...
        >
          ...
          >
            <div className="p-4 border-b border-border flex justify-between items-center">
              <h2 className="font-semibold">내가 보낸 질문 ({sentQuestions.length})</h2>
              <button
                onClick={() => setShowAllQuestions(false)}
